...
\begin{abstract}
Large language model (LLM) agents are transitioning from query engines to autonomous participants in negotiations, coalitions, and social dilemmas. A striking empirical pattern is emerging across this new behavioural substrate: strategic dispositions, theory-of-mind performance, and cooperative tendencies shift systematically with the language of interaction --- what recent work has called an ``Algorithmic Sapir--Whorf Switch''. This perspective synthesises three converging lines of inquiry to argue that language is not a passive interface for machine agents but an active modulator of their behaviour. First, we characterise the behavioural phenotype: cooperation rates, defection patterns, and commitment reliability vary across languages, including a ``nicer than human'' prosocial bias ($\sim 79\%$ cooperation, well above the $\sim 48\%$ human baseline) that is itself language-dependent. Second, we connect these shifts to mechanistic accounts --- the Semantic Hub and Translation Barrier hypotheses from interpretability research --- and to cognitive findings on multilingual theory-of-mind deficits in benchmarks such as XToM. Third, we situate LLM cooperation within the evolutionary game-theoretic tradition of Axelrod and contemporary cultural-evolution research, asking how language-modulated machine behaviour will shape, and be shaped by, the multilingual human institutions into which these agents are deployed. We foreground contradictory evidence --- a cross-lingual persuasion study reporting null effects on human outcomes, bimodal Arabic strategy distributions, and Vietnamese subverting collectivist priors --- to caution against cultural essentialism and to motivate a science of multilingual machine behaviour. We close with open questions on alignment, evaluation reliability, and the co-evolution of LLM agents with diverse human linguistic communities.
\end{abstract}
...
A generation of computational models that once produced text on demand now \emph{acts}: LLM agents negotiate trades, allocate resources, deceive opponents in social-deduction games, and collaborate on open-ended tasks over hours-long horizons \cite{meta2022cicero, wang2024voyager, huynh2026moreatstake, sun2025gametheoryllm}. This transition invites, and arguably requires, a new research programme: the \emph{machine-behaviour} paradigm proposed by Rahwan and colleagues \cite{rahwan2019machinebehaviour}, which treats AI systems as a class of behaving agents whose ethology must be studied empirically, with tools drawn from biology, economics, cognitive science and the social sciences, rather than solely from the engineering discipline that built them.
...
Against these divergences sits a baseline finding that has not received the attention it deserves from the machine-behaviour community. LLMs in one-shot and iterated social dilemmas default to \emph{prosocial} play at rates far above human baselines --- cooperation rates of $\sim 79\%$ in controlled IPD comparisons, against a historical human rate of $\sim 48\%$ \cite{fontana2024nicer, za2026towards, wang2026discovering}. This is striking evolutionarily: thirty years of agent-based models following Axelrod \cite{axelrod1984evolution} suggest that stable cooperation requires repeated interactions, reputation, and punishment. LLM cooperation is observed even when these scaffolds are weak, suggesting that cultural inheritance via pretraining produces a default disposition that evolutionary game theory would not predict from interaction structure alone. We return to this in Section~\ref{sec:evolution}.

\subsection{Contradictory Evidence and Confounds}
\label{sec:confounds}

A responsible account of linguistic relativity in machines must foreground the evidence against it. Three findings are central. First, in a rigorous cross-lingual persuasion study, \citeauthor{arango2024persuasion} \cite{arango2024persuasion} find \emph{no significant effect} of LLM-authored versus human-authored charitable appeals on human persuasive outcomes across English and German. Human recipients anchor on source belief (``did an AI write this?'') rather than linguistic nuance. Second, when game horizons are unknown, cooperation \emph{universalises} across all languages \cite{buscemi2025fairgame, lore2024framing}: framing effects can fully dominate language effects. Third, apparent Vietnamese ``defection'' in low-stakes rounds sometimes reflects arithmetic miscomprehension under attenuated payoffs rather than strategic intent \cite{huynh2025fairgame}.
...
The \emph{Semantic Hub Hypothesis} \cite{wu2024semantichub}, supported by logit-lens decoding \cite{wendler2024llamasenglish}, holds that multilingual LLMs project heterogeneous linguistic inputs into a centralised, English-dominant representational hub in intermediate layers. Non-English inputs undergo lossy compression before strategic conclusions are computed in English-anchored hidden states. Activation Addition (ActAdd) experiments offer suggestive functional evidence: English-derived steering vectors require scaling coefficients approximately 2--3$\times$ larger to alter non-English outputs \cite{wu2024semantichub, turner2023actadd}. We note that these coefficients are reported on a limited set of model families and layers, and require independent replication.
...
SPIN-Bench sharpens the picture by decoupling planning from social intelligence \cite{yao2025spinbench}: o3-mini scores over $51\%$ on formal PDDL planning yet fails at Diplomacy intention inference, while Claude 3.5 Sonnet inverts the pattern. Planning and ToM are distinct faculties that do not scale together. From a machine-behaviour standpoint, this mirrors the evolutionary-psychology distinction between ``Machiavellian intelligence'' and domain-general problem solving \cite{byrne1988machiavellian}, and suggests that cross-lingual transfer may be uneven across these faculties in ways that demand separate measurement.

\section{Evolutionary Perspectives on Machine Cooperation}
\label{sec:evolution}

The ``nicer than human'' finding (Section~\ref{sec:relativity}) demands an evolutionary-game-theoretic explanation, and connecting LLM behaviour to this tradition is where the machine-behaviour research programme is least developed and most promising.
...
Language interacts with this process. If the transmission bottleneck is largely English --- because English is the lingua franca of benchmarks, distillation targets, and RLHF annotator pools --- then non-English behavioural traits may drift independently, neither as evolved nor as selected, simply as under-observed side effects. The Semantic Hub Hypothesis supplies the architectural substrate for this prediction \cite{wu2024semantichub}. Empirically, the widening English-to-Arabic gap in TCG-Bench at 32B parameters \cite{wang2025tcgbench} is consistent with a drift that scale does not correct.

\subsection{Trust, Reciprocity, and Commitment}

The four CFP concepts meet in this section. \emph{Trust} and \emph{reciprocity} are operationalised in commitment reliability \cite{xie2026m3bench}: when agents announce cooperation and then defect, trust among machine and human counterparts erodes. \emph{Cooperation} is the behavioural output measured in Table~\ref{tab:strategy} and the ``nicer than human'' finding \cite{fontana2024nicer}. \emph{Cultural evolution} is the transmission mechanism producing these behaviours without in-game selection. Framing LLM social behaviour in these terms makes it commensurable with a century of work in biology, anthropology, and economics, rather than a brand-new object requiring a brand-new vocabulary.

\section{Societal Implications and Open Questions}
\label{sec:society}

Multilingual LLM agents are not a laboratory curiosity. They are being deployed, now, in institutional contexts where language is the primary mediator of human outcomes: education platforms in Swahili and Hindi, public-sector chatbots in Arabic and Vietnamese, customer-service agents across dozens of markets, and increasingly, autonomous multi-agent systems negotiating commercial transactions \cite{martinez2024algorithmictrading, yong2025multilingualsafety}. If language modulates machine behaviour as Sections~\ref{sec:relativity}--\ref{sec:evolution} suggest, three societal consequences warrant attention.
...
Four questions strike us as most consequential.

\emph{(i) Causal isolation of language.} Can the Algorithmic Sapir--Whorf Switch be demonstrated under controlled confound ablation --- matched tokenisers, audited RLHF coverage, pre-registered hypotheses? Current evidence is convergent but not causally clean.

\emph{(ii) Mechanistic diagnostics at scale.} Can the hub--barrier distinction be turned into a standard benchmarking protocol (intermediate-layer probes at $\lfloor L/3 \rfloor$ and $\lfloor 2L/3 \rfloor$, with pre-registered acceptance criteria) \cite{kumar2025indictunedlens}? If so, practitioners can triage failures into training-data versus generation-stage remedies.

\emph{(iii) Evaluation reliability.} Multilingual LLM-as-Judge reliability sits at Fleiss' $\kappa \approx 0.3$ (``fair to poor'') \cite{fu2025multilingualjudge}; panels of $\geq 3$ heterogeneous judges with self-translation and checklist rubrics can recover above $\kappa \approx 0.5$. Closing this gap is prerequisite for any quantitative machine-behaviour science.

\emph{(iv) Cultural-evolution dynamics.} How do behavioural traits propagate through inter-model distillation and through model--human corpus loops, and does the dynamic preserve or erode linguistic diversity in the behavioural phenotype? This is perhaps the most natural place for the machine-behaviour programme to collaborate with cultural evolutionary biology.

\section{Conclusions}

We have argued that language is an active modulator of behaviour in LLM agents, and that understanding this modulation requires integrating three traditions: \emph{behavioural characterisation} in the style of Rahwan's machine-behaviour programme \cite{rahwan2019machinebehaviour}; \emph{mechanistic interpretability}, which supplies the Semantic Hub and Translation Barrier accounts \cite{wu2024semantichub, bafna2025translationbarrier}; and \emph{evolutionary and cultural-evolution theory}, which provides the vocabulary of trust, reciprocity, inherited cooperation, and transmission dynamics \cite{axelrod1984evolution, henrich2016secret}. The emergent phenomena --- ``nicer than human'' cooperation that erodes language-dependently, asymmetric theory-of-mind degradation, compounding safety vulnerabilities via code-switching --- are not isolated benchmark artefacts but coordinated signatures of a multilingual machine ethology that is only beginning to be measured.

We have been careful to foreground contradictory evidence: a persuasion null finding, framing effects that dominate language effects, and unresolved confounds. The Algorithmic Sapir--Whorf Switch remains a working hypothesis, not a proven law. Yet the evidence is sufficient to motivate a research programme --- one that the Royal Society's long-running engagement with behaviour, cooperation, and evolution seems uniquely positioned to host.
